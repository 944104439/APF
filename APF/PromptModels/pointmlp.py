...
def simplied_morton_sorting(center):
    ...
    # print(distances_batch.shape) # 2,1024,1024
    distances_batch[:, torch.eye(num_group).bool()] = float("inf")

    idx_base = torch.arange(
        0, batch_size, device=center.device) * num_group
    sorted_indices_list = []
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        idx = knn_point(self.kneighbors, xyz, new_xyz) # B G M
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        # print(last_channel)
        self.act = get_activation(activation)
        self.tokenize = nn.Linear(2048,768)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        x = self.tokenize(x.permute(0,2,1))
        return x


def pointMLP(num_classes=40, **kwargs) -> Model:
    return Model(points=1568, class_num=num_classes, embed_dim=128, groups=1, res_expansion=1.0,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                   k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 1], **kwargs)


def pointMLPElite(num_classes=40, **kwargs) -> Model:
    return Model(points=1568, class_num=num_classes, embed_dim=32, groups=1, res_expansion=0.25,
                 activation="relu", bias=False, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2], pre_blocks=[1, 1, 2], pos_blocks=[1, 1, 2],
...
if __name__ == '__main__':
    data = torch.rand(2, 2048, 4).to('cuda')
